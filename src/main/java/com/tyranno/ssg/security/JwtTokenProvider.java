...
public class JwtTokenProvider {
    ...
    public String validateAndGetUserUuid(String token) { //토큰을 검증하고 유효한 경우 토큰에 담긴 사용자 UUID를 반환합니다. 토큰이 유효하지 않은 경우, 로그를 남기고 null을 반환
        try {
            return extractClaim(token, Claims::getSubject);
    ...
    private Claims extractAllClaims(String token) { // 토큰에서 모든 클레임을 추출
        return Jwts
                .parserBuilder()
